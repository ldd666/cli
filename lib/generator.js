...
class Generator {
  ...
  extendPackages () {
    ...
    for (const dep in this.presetPkg.devDependencies) {
      if (!packages[dep]) {
        packages.devDependencies[dep] = this.presetPkg.devDependencies[dep].version
      }
    }
    for (const dep in this.presetPkg.dependencies) {
      if (!packages[dep]) {
        packages.dependencies[dep] = this.presetPkg.dependencies[dep].version
      }
    }
  ...
  init (presets) {
    ...
    presets.features.forEach(preset => {
      ...
      // init package
      if (this.presetCfg[preset].devDependencies && Array.isArray(this.presetCfg[preset].devDependencies)) {
        this.presetCfg[preset].devDependencies.forEach(item => {
          if (!item.name) return
          this.presetPkg.devDependencies[item.name] = item
        })
      }
      if (this.presetCfg[preset].dependencies && Array.isArray(this.presetCfg[preset].dependencies)) {
        this.presetCfg[preset].dependencies.forEach(item => {
          if (!item.name) return
          this.presetPkg.dependencies[item.name] = item
        })
      }
      // 将模板文件 copy 到项目路径下面
